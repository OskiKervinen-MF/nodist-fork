...
NPMIST.latestVersion = function(){
  // The npm repo has releases of utilities, so we can't
  // just blindly use getLatestRelease. We must get all
  ...
  .then((releases) => {
    ...
    // Ignore the releases of libraries. They are named "package: version",
    // while core npm releases are named just "version".
    releases = releases.filter( release => release.name.indexOf(':') < 0 );

    // Check we found at least one.
    if( releases.length < 1 )
      throw "No npm/cli releases found.";

    return releases.sort( (r1, r2) => r1.created_at - r2.created_at )[0].tag_name;
  });
};
